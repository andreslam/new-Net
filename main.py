...
if isTraining:  # train
    ...
    # 加载数据集
    train_dataloader = DataLoader(database, batch_size=args.batch_size, num_workers=0, shuffle=True)
    ...
    # first_net = NNet(n_channels=args.input_nc, n_classes=1).to(device)
    ...
    net_criterion = torch.nn.MSELoss()
    ...
    for epoch in range(args.first_epochs):
        ...
        print('-' * 10)
        first_net = train_first_stage(viz, writer, train_dataloader, first_net, first_optim, args.init_lr,net_criterion,
                                      device, args.power, epoch, args.first_epochs)
        if (epoch + 1) % args.val_epoch_freq == 0 or epoch == args.first_epochs - 1:
            first_net, best_fusion = val_first_stage(best_fusion, viz, writer, val_dataloader, first_net,
                                                     net_criterion,
    ...
    = test_first_stage(val_dataloader, first_net, device,  args.results_dir + "/" + sub_dir,
                        fusion_criterion)
    print("loss_dct, auc_dct, acc_dct, sen_dct, fdr_dct, spe_dct, kappa_dct, gmean_dct, iou_dct, dice_dct "+loss_dct+auc_dct+acc_dct+sen_dct+spe_dct+kappa_dct+gmean_dct+iou_dct+dice_dct)
else:  # test
    # 加载数据集和模型
    ...
    # start testing
    print("Start testing...")
    test_first_stage(test_dataloader, net, device, args.results_dir
