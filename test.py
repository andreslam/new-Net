...
def get_results(loss_lst, auc_lst, acc_lst, sen_lst, fdr_lst, spe_lst, kappa_lst, gmean_lst, iou_lst, dice_lst, dataloader, results_dir, criterion, pred, gt, mask_arr=None, isSave=True):
    ...
    if isSave:
        ...
        pred_img2=cv2.normalize(pred_img,None,0,255,cv2.NORM_MINMAX)
        # cv2.imshow('test',pred_img)
        # key=cv2.waitKey(0)
        plt.axis('off')
        # plt.imshow(pred_img, cmap="gray")
        # print(results_dir + imgName)
        cv2.imwrite(results_dir+imgName, pred_img)
        path = results_dir  # 图片路径

        img.imsave(results_dir+imgName, pred_img)
        # cv2.imwrite(results_dir + "" + imgName, thresh_pred_img)
...
def test_first_stage(dataloader, net, device, results_dir,
    ...
    loss_dct = { "fusion": []}  # Loss
    auc_dct = { "fusion": []}  # AUC
    acc_dct = {"fusion": []}  # Accuracy
    sen_dct = { "fusion": []}  # Sensitivity (Recall)
    fdr_dct = { "fusion": []}  # FDR
    spe_dct = { "fusion": []}  # Specificity
    kappa_dct = { "fusion": []}  # Kappa
    gmean_dct = { "fusion": []}  # G-mean
    iou_dct = { "fusion": []}  # IOU
    dice_dct = {"fusion": []}  # Dice Coefficient (F1-score)
    ...
    with torch.no_grad():
        for sample in dataloader:
            ...
            #             gt_criterion, pred, gt, mask_arr=mask_arr)
            get_results(loss_dct["fusion"], auc_dct["fusion"], acc_dct["fusion"], sen_dct["fusion"],
                        ...
                        dataloader, results_dir , fusion_criterion, pred, gt, mask_arr=mask_arr)
    ...
    # print_results(loss_dct, auc_dct, acc_dct, sen_dct, fdr_dct, spe_dct, kappa_dct, gmean_dct, iou_dct, dice_dct)
...
def test_second_stage(dataloader,first_net_thick, first_net_thin, fusion_net, device, results_dir, criterion=None, isSave=True):
    ...
    with torch.no_grad():
        for sample in dataloader:
            ...
            pred = fusion_net(img[:, :1, :, :], thick_pred[0], thin_pred[1])
            if len(sample) == 5:
                w, h = sample[4]
